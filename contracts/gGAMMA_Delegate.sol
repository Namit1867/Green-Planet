...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_WITHDRAW_FEE_FACTOR_ACCRUE_INTEREST_FAILED,
        SET_WITHDRAW_FEE_FACTOR_ADMIN_CHECK,
        SET_WITHDRAW_FEE_FACTOR_FRESH_CHECK,
        SET_WITHDRAW_FEE_FACTOR_BOUNDS_CHECK,
...
contract GToken is GTokenInterface, Exponential, TokenErrorReporter {
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        if (withdrawFeeFactor < withdrawFeeFactorMax && vars.redeemAmount != 0) {
            
            
            uint actualRedeemAmount = vars.redeemAmount;
            
            (,vars.redeemAmount) = mulUInt(withdrawFeeFactor,vars.redeemAmount);
            
            (,vars.redeemAmount) = divUInt(vars.redeemAmount,withdrawFeeFactorMax);
            
            uint fee = 0;
            (,fee) = subUInt(actualRedeemAmount,vars.redeemAmount);
    ...
    function _setWithdrawFeeFactor(uint newWithdrawFeeFactor) external nonReentrant returns (uint) {
        ...
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but on top of that we want to log the fact that an attempted reserve factor change failed.
            return fail(Error(error), FailureInfo.SET_WITHDRAW_FEE_FACTOR_ACCRUE_INTEREST_FAILED);
        }
        // _setReserveFactorFresh emits reserve-factor-specific logs on errors, so we don't need to.
    ...
    function _setWithdrawFeeFactorFresh(uint newWithdrawFeeFactor) internal returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_WITHDRAW_FEE_FACTOR_ADMIN_CHECK);
        }
        // Verify market's block number equals current block number
        if (accrualBlockNumber != getBlockNumber()) {
            return fail(Error.MARKET_NOT_FRESH, FailureInfo.SET_WITHDRAW_FEE_FACTOR_FRESH_CHECK);
        }
        ...
        if (newWithdrawFeeFactor > withdrawFeeFactorMax || newWithdrawFeeFactor < withdrawFeeFactorLL) {
            return fail(Error.BAD_INPUT, FailureInfo.SET_WITHDRAW_FEE_FACTOR_BOUNDS_CHECK);
        }
...
// File: lending/GErc20Delegate.sol
