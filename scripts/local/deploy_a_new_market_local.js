...
async function main() {
  ...
  if(oracle_address === ""){
    ...
    priceOracle = await PriceOracle.connect(signer).deploy(BigInt(anchorToleranceMantissa_),anchorperiod,configs);
   ...
   for(var i = 0 ; i < Object.keys(price_configs).length ; i++){
    ...
    await unitroller.connect(signer)._setPriceOracle(priceOracle.address);
  
    await planetDiscountDelegator.connect(signer).changeAddress(await planetDiscountDelegator.gGammaAddress(),unitroller.address,priceOracle.address)
    ...
    for(var i = 0 ; i < Object.keys(price_configs).length ; i++){
      const config = await priceOracle.getTokenConfigByGToken(price_configs[i].delegator);
      const gToken = config.gToken;
      await priceOracle.validate(gToken);
      console.log(`\n ${gToken} PRICE `," : ","$",await priceOracle.getUnderlyingPrice(gToken) / 1e18,"\n");
   }
  }
  ...
  for (var i = 0; i < Object.keys(token_configs).length; i++) {
    ...
    const gtokendelegate = await gTokenDelegate.connect(signer).deploy();

    await gtokendelegate.connect(signer).deployed();
    ...
    const gtokendelegator = await gTokenDelegator.connect(signer).deploy(
    ...
    await gtokendelegator.connect(signer).deployed();

    await unitroller.connect(signer)._supportMarket(gtokendelegator.address);
    ...
    await priceOracle.connect(signer).validate(gtokendelegator.address);

    await unitroller.connect(signer)._setCollateralFactor(
      gtokendelegator.address,
      BigInt(token_configs[i].collateral)
    );

    await gtokendelegator.connect(signer)._setDiscountLevel(planetDiscountDelegator.address);

    await gtokendelegator.connect(signer)._setReserveFactor(
      BigInt(token_configs[i].reserveFactor)
    );

    await planetDiscountDelegator.connect(signer).listMarket(gtokendelegator.address);
